...
std::pair<double,double> RecombinationHistory::electron_fraction_from_saha_equation(double x) const{
  ...
  Xe = -b/2 + sqrt(pow(b/2., 2.) + b); // 2./(sqrt(1.+4./b) + 1.)
  ne = Xe*nH;
...
int RecombinationHistory::rhs_peebles_ode(double x, const double *Xe, double *dXedx){
  ...
  double alpha_2 = c*8./sqrt(3.*M_PI) * sigma_T * sqrt(epsilon_0/(Tb*k_b))*phi_2;
  double beta = alpha_2*(m_e*Tb/(2.*M_PI))*exp(-epsilon_0/(Tb*k_b));
  double beta_2 = beta*exp(3.*epsilon_0/(4.*Tb));
  //const double rho_c = 3.*pow(cosmo->get_H0(), 2.)/(8.*M_PI*G);
  //double nb = OmegaB0*rho_c/(m_H*pow(a, 3.));
  double nH = (1. - Yp)*nb;
  double n_1s = (1. - X_e)*nH;
  double lambda_a = H * pow(3*epsilon_0, 3.)/(pow(8.*M_PI, 2.)*n_1s);
